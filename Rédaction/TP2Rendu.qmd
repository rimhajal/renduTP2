...
Comme dans la première partie, les erreurs des deux critères se comportent de la même manière. 
On note qu'avec une taille de n = 160 < 456, l'erreur s'annule plus rapidement. Dès une profondeur maximale de 9, il est clair qu'il sera inutile d'augmenter la profondeur vue que l'erreur est pratiquement stable à ce point là.

## Dataset `DIGITS`
<a name="Dataset `DIGITS`"></a>

On effectue encore une fois la même étude mais cette fois-ci sur les données `DIGITS` disponibles sur Python dans le module `sklearn.datasets`.
On découpe tout d'abord les données en "testing set" et "training set" et ceci à travers la fonction `train_test_split`.
...
Ce qui est plus important, c'est l'erreur avec les données test :
...
Avec les données test, on trouve une différence avec l'entropie et l'indice de gini. En effet, l'entropie aboutit à de meilleurs résultats et des erreurs plus petites que celles obtenues avec l'indice de gini. De plus, la chute des erreurs est plus rapide avec l'entropie qu'avec l'indice de gini.
D'après le graphe, on peut dire que 6 est un bon choix de profondeur.
